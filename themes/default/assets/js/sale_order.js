...
$(document).on('keyup', '#amount_1', function () {
	...
	var old_amount = $('#amount_1').attr('old_amount');
	...
	if(us_paid < old_amount){
		var last_deposit_balance = deposit_balance -  us_paid;
		$('.deposit_total_balance').text(formatMoney(last_deposit_balance,'$'));
		
	}else{
		$("#amount_1").val(old_amount);
		$('.deposit_total_balance').text(formatMoney(last_deposit_balance,'$'));
	}
	
	return false;
	deposit_balance = formatDecimal(deposit_amount - us_paid);
	if(deposit_balance < 0) {
		$(".deposit_total_balance").text(0);
		$("#amount_1").val(formatMoney(deposit_amount));
		$("#amount_1").select();
	}else {
				...
				if (data === false) {
					...
					if(payment_status == "paid"){	
						if(deposit_balance<amount){
							...
							$("#amount_1").attr('old_amount', deposit_balance);
							$("#amount_1").attr('old_balance', deposit_balance);	
						}else{
							$("#amount_1").val(amount);
						...
						$('#dp_details').html('<small>Customer Name: ' + data.company + '<br>Amount: <span class="deposit_total_amount">' + formatMoney(deposit_amount,'$') + '</span> - Balance: <span class="deposit_total_balance">' + formatMoney(deposit_balance,'$') + '</span></small>');
...
function loadItems() {
	if (localStorage.getItem('sloitems')) {
		...
		$.each(sloitems, function () {
				...
				if (pr_tax !== false) {
					if (pr_tax.type == 1) {
						...
						if (item_tax_method == '0') {
							...
							//pr_tax_val = (formatDecimal((unit_price) * (parseFloat(pr_tax.rate)/ 100))).toFixed(2);
			...
			if(!item_discount) {
				if(item_price == 0){
					item_price = item.group_price[0].price ;
				}
			}
			...
			if (sltax2 = localStorage.getItem('sltax2')) {
				$.each(tax_rates, function () {
					if (this.id == sltax2) {
						...
						if (this.type == 1) {
							invoice_tax = formatDecimal(((total - order_discount + shipping) * this.rate) / 100);
						}
					}
				});
			}
