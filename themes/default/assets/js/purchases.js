...
function loadItems() {
    if (localStorage.getItem('poitems')) {
        ...
        $.each(poitems, function () {
			...
			if(parseFloat(item.row.received) == 0){
				return false;
			}
			...
			tr_html = '<td class="text-right"><span class="text-center">#'+ no_ +'<input name="create_id[]" type="hidden" class="create_id" value="' + create_id + '"><input name="pur_id[]" type="hidden" class="pur_id" value="' + pur_id + '"><input name="create_order_id[]" type="hidden" class="create_order_id" value="' + create_order_id + '"><input name="pur_order_id[]" type="hidden" class="pur_order_id" value="' + pur_order_id + '"></td>';
