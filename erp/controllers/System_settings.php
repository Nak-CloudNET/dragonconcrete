...
class system_settings extends MY_Controller
{
	...
	function getProductPrices($group_id = NULL)
	{
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select("{$this->db->dbprefix('products')}.id as id, {$this->db->dbprefix('products')}.code as product_code, {$this->db->dbprefix('products')}.name as product_name, PP.currency_code AS currency ,PP.price as price ")
            ->from("products")
