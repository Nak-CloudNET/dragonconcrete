        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('C2', lang('shop'));
                    $this->excel->getActiveSheet()->SetCellValue('D2', lang('customer'));
                    $this->excel->getActiveSheet()->SetCellValue('E2', lang('sale_status'));
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('I2', lang('payment_status'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->getStyle("A" . $row . ":I" . $row)->getFont()->setSize(18);
                        $row++;
                    }
                    ...
                    if ($this->input->post('form_action') == 'export_pdf') {
                        ...
                        $this->excel->getActiveSheet()->getStyle("A2:I2")->getFont()->setSize(13);
                    ...
                    if ($this->input->post('form_action') == 'export_excel') {
                        ...
                        $this->excel->getActiveSheet()->getStyle("A" . $row . ":I" . $row)->getFont()->setSize(16);

                        $this->excel->getActiveSheet()->getStyle("A1:I1")->getBorders()
                            ->getBottom()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
                        $this->excel->getActiveSheet()->getStyle('A1:I1')->applyFromArray($styleArray);
                        $this->excel->getActiveSheet()->getStyle('A2:I2')->applyFromArray($styleArray);
                        $this->excel->getActiveSheet()->getStyle("A1:I1")->getFont()->setSize(22);
                        $this->excel->getActiveSheet()->getStyle("A2:I2")->getFont()->setSize(20);
                        $this->excel->getActiveSheet()->getStyle('A2:I2')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    ...
    function return_sale($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->returnSale($data, $products, $payment)) {
            ...
            foreach ($inv_items as $item) {
                ...
                $row->unit_price = $row->tax_method ? $item->unit_price+$this->erp->formatDecimal($item->item_discount/$item->bqty)+$this->erp->formatDecimal($item->item_tax/$item->bqty) : $item->unit_price+($item->item_discount/$item->bqty);
                $row->real_unit_price = $item->real_unit_price;
				$row->cost = (($item->unit_cost > 0) ? $item->unit_cost : $row->cost);
                $row->tax_rate = $item->tax_rate_id;
                $row->serial = $item->serial_no;
                $row->option = $item->option_id;
                $options = $this->sales_model->getProductOptions($row->id, $item->warehouse_id, TRUE);
        ...
        if ($this->form_validation->run() == true) {            
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $this->excel->setActiveSheetIndex(0);
                    $this->excel->getActiveSheet()->setTitle(lang('sales'));
                    $this->excel->getActiveSheet()->SetCellValue('G1', lang('List POS'));
                    $this->excel->getActiveSheet()->SetCellValue('A2', lang('date'));
    ...
    '</ul>
        ...
        if ($biller_id) {
    		$this->datatables
                ...
                $this->datatables
                ->select("erp_deliveries.id AS id,erp_deliveries.date,erp_deliveries.do_reference_no,erp_deliveries.sale_reference_no,erp_cust.name AS customer_name,erp_cust.address,SUM(IF(
            erp_delivery_items.option_id,
            (
                erp_sale_items.quantity*qty_mouy.qty_unit
            ),
            erp_sale_items.quantity
        )
    ) AS qto,SUM(IF(
                erp_delivery_items.option_id,(qty_mouy.qty_unit*erp_delivery_items.quantity_received), erp_delivery_items.quantity_received) ) as qty,erp_deliveries.delivery_status AS de_sale_status")
                ->from('deliveries')
                ->join('erp_delivery_items', 'erp_delivery_items.delivery_id = erp_deliveries.id', 'left')
                ->join('erp_sale_items', 'erp_sale_items.product_id = erp_delivery_items.product_id', 'left')
                ->join('companies as erp_cust', 'cust.id = deliveries.customer_id', 'left')
                ->join('(SELECT * from erp_product_variants) AS qty_mouy','erp_delivery_items.option_id = qty_mouy.id','left')
                ->where('type','invoice')
                ->group_by('deliveries.id');
        }
									...
									if (!empty($product['product_code']) && !empty($product['unit_price']) && !empty($product['quantity'])) {
										if ($product_details = $this->site->getProductByCode(trim($product['product_code']))) {
											...
											if($product['variant_id']) {
												...
												$option = $this->site->getProductVariant($variant->name, $item_id);
	...
	function getSales_items($start = NULL, $end = NULL)
    {
        ...
        if ($biller_id) {
            ...
            erp_sale_items.quantity
                ...
                erp_delivery_items.option_id,(qty_mouy.qty_unit*erp_delivery_items.quantity_received), erp_delivery_items.quantity_received) ) as qty,
    					COALESCE(SUM(erp_sale_items.quantity),0) - COALESCE(SUM(erp_sale_items.quantity_received),0) as balance,
    					(IF(ISNULL(".$this->db->dbprefix("sales").".delivery_status), CONCAT(erp_sales.id, '___', 'delivery'),
    					CONCAT(erp_sales.id, '___', ".$this->db->dbprefix("sales").".delivery_status))) as delivery_status")
                ->from('sales')
    			->join('users','sales.saleman_by=users.id','left')
                ->join('delivery_items','delivery_items.sale_id=sales.id','left')
    			->join('companies as erp_pro', 'pro.id = sales.biller_id', 'left')
    			->join('sale_items','sales.id=sale_items.sale_id','left')
                ->join('(SELECT * from erp_product_variants) AS qty_mouy','delivery_items.option_id = qty_mouy.id','left')
...
//$this->erp->print_arrays($delivery);
    ...
    function getHouse_calendar()
    {
        ...
        $this->datatables
            ->select("products.id as id, products.name, IF(erp_sales.id, erp_sales.customer, erp_sale_order.customer) as customer_name, 
					...
					CASE WHEN erp_products.id = erp_sale_items.product_id THEN 'sold' WHEN erp_products.id = erp_sale_order_items.product_id THEN 'order' ELSE 'aval' END AS status, sales.attachment as attachment")
    ...
    function invoice_a4($id=null)
    {
        ...
        $this->data['title'] = "2";
        $this->data['sid'] = $id;
        $this->load->view($this->theme .'sales/invoice_a4',$this->data);
    }
	...
	function down_payment($id=null)
	{
        ...
        if ($this->form_validation->run() == true) {
				...
				$depre_rate    = $this->input->post('depreciation_rate_1');
    ...
    {
	//	$this->erp->print_arrays($this->sales_model->getJoinlease($id));
		$months = array(
                '01' => 'មករា',
                '02' => 'កុម្ភៈ',
                '03' => 'មិនា',
		...
		// $this->erp->print_arrays($this->data['saller']);
