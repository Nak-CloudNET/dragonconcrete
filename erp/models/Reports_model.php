...
//				 $this->db->group_by('erp_deliveries.id');
    ...
    public function getSaleman($start_date=null, $end_date=null, $saleman=null)
    {
    	$this->db
    ...
    public function getSalemanReportDetail($saleman_id, $start_date2 = NULL, $end_date2 = NULL, $saleman2 = NULL){
...
*/
   ...
   public function getCustomersByID($id=null, $wh=null) {
	...
	public function getCategoriesInventoryValuationByWarehouse($warehouse_id,$category_id,$product_id,$stockType,$from_date,$to_date,$reference_no, $biller, $plan_id){
	...
	public function getProductsInventoryValuationByProduct($warehouse_id,$category_id,$product_id,$stockType,$from_date,$to_date,$reference_no, $biller, $plan_id){
		$this->db->select('inventory_valuation_details.*, companies.name AS biller_name, project_plan.plan as home_type');
		...
		if($biller){
			$this->db->where('inventory_valuation_details.biller_id', $biller);
		}
		if($reference_no){
			$this->db->where('inventory_valuation_details.reference_no', $reference_no);
		}
		if($plan_id){
			$this->db->where('inventory_valuation_details.plan_id', $plan_id);
		}
		if($from_date && $to_date){
			$this->db->where('date_format(erp_inventory_valuation_details.date,"%Y-%m-%d") >="'.$from_date.'" AND date_format(erp_inventory_valuation_details.date,"%Y-%m-%d") <="'.$to_date.'"');
		}
	...
	public function getWarePur($wid,$warehouse,$product,$category,$biller){
		...
		->where('erp_purchase_items.status !=', 'pending');
		if($warehouse){
	...
	public function getQtyINALL($id,$wid,$tr,$start,$end,$biller){
		...
		$this->db->where('purchase_items.date >="'.$start.'" AND purchase_items.date<="'.$end.'"');
		$this->db->where(array("product_id"=>$id,"transaction_type"=>$tr,"purchase_items.warehouse_id"=>$wid));
		$q = $this->db->get("purchase_items");
		if ($q->num_rows() > 0) {
            return $q->row();
        }
	...
	public function getAmountQtyINALL($id,$wid,$tr,$cid,$start,$end){
		...
		$this->db->where('purchase_items.date >="'.$start.'" AND purchase_items.date<="'.$end.'"');
		$this->db->where(array("transaction_type"=>$tr,"erp_purchase_items.warehouse_id"=>$wid,"products.category_id"=>$cid));
		$q = $this->db->get("purchase_items");
		if ($q->num_rows() > 0) {
            return $q->row();
        }
	...
	public function getAmountQtyINALLCAT($id,$wid,$tr,$cid,$start,$end,$biller){
		...
		$this->db->where('purchase_items.date >="'.$start.'" AND purchase_items.date<="'.$end.'"');
		$this->db->where(array("transaction_type"=>$tr,"erp_purchase_items.warehouse_id"=>$wid));
		$q = $this->db->get("purchase_items");
		if ($q->num_rows() > 0) {
            return $q->row();
        }
	...
	public function getBeginQtyINALL($id,$wid,$start,$end,$biller){
		...
		$this->db->where('purchase_items.date <"'.$start.'" ');
	...
	public function getQtyByWare($pid,$wid,$product2,$category2,$start,$end,$biller, $expiry){
		$this->db->select("SUM(COALESCE(erp_purchase_items.quantity_balance,0)) as wqty");
		$this->db->where("purchase_items.status !=", "pending");
	...
	public function getQtyUnitINALL($id,$wid,$tr,$start,$end,$biller){
		...
		$this->db->where("quantity_balance>",0);
		$this->db->where('purchase_items.date >="'.$start.'" AND purchase_items.date<="'.$end.'"');
		$this->db->where(array("erp_purchase_items.product_id"=>$id,"transaction_type"=>$tr,"erp_purchase_items.warehouse_id"=>$wid));
	...
	public function getBeginQtyALL($id,$wid,$start,$end,$biller){
		...
		$this->db->where('purchase_items.date >="'.$startDate.'" AND purchase_items.date<="'.$endDate.'"');
		$this->db->where(array("erp_purchase_items.product_id"=>$id,"purchase_items.warehouse_id"=>$wid));
