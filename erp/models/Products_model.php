...
class Products_model extends CI_Model
{
	...
	public function getConvert_ItemByID($id, $ware_id = NULL)
    {
		$this->db->select('convert_items.id, 
							...
							(SELECT COALESCE(quantity , 0) as qoh FROM erp_warehouses_products WHERE warehouse_id = '.$ware_id.' AND erp_warehouses_products.product_id = erp_convert_items.product_id) as qoh
							');
    ...
    {
        $this->db->select($this->db->dbprefix('products') . '.id, code, ' . $this->db->dbprefix('products') . '.name as name, (SELECT COALESCE(quantity , 0) as qoh FROM erp_warehouses_products WHERE warehouse_id = '.$warehouse_id.' AND erp_warehouses_products.product_id = erp_products.id GROUP BY erp_warehouses_products.product_id) as qoh')
				 ->where("type != 'combo' AND " . "(" . $this->db->dbprefix('products') . ".name LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR concat(" . $this->db->dbprefix('products') . ".name, ' (', code, ')') LIKE '%" . $term . "%') AND inactived <> 1")
				->limit($limit);
        $q = $this->db->get('products');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
    ...
    public function addProduct($data, $items, $product_attributes, $photos, $related_products=NULL, $items2)
    {
        ...
        if ($this->db->insert('products', $data)) {
			...
			if($data['service_type'] == 1){
				$purcahse_item = array(
					...
					'date' 				=> date('Y-m-d'),
					'status' 			=> 'received',
					'transaction_type'	=> 'HOME SERVICE',
					'transaction_id'	=> $product_id
				);
				$this->db->insert('purchase_items', $purcahse_item);
	...
	public function getUsingStockProducts($term, $warehouse_id, $plan = NULL, $limit = 100)
    {
		$project_plan_qty = 0;
		...
		if ($plan) {
			$project_plan_qty = "(SELECT product_id, COALESCE(COALESCE(quantity_balance, 0) - COALESCE(quantity_used, 0)) as project_qty FROM erp_project_plan_items WHERE project_plan_id = $plan ) project_plan";
		}
		$this->db->where("warehouses_products.warehouse_id", $warehouse_id);
        $this->db->limit($limit);
		if ($plan) {
			$this->db->select('products.*, warehouses_products.quantity as qoh, units.name as unit_name, project_plan.project_qty');
		} else {
    ...
    {
        $this->db->select("warehouses.id as id, sale_order.reference_no, warehouses.name, ((erp_sale_order_items.quantity * COALESCE(erp_product_variants.qty_unit, 1)) - (erp_sale_order_items.quantity_received * COALESCE(erp_product_variants.qty_unit, 1))) as qty, sale_order.sale_status")
		...
		if($this->db->insert('enter_using_stock', $data)){
			...
			foreach($items as $item){
				...
				$pur_data = array(
					...
					'reference'			=> $item['using_reference_no'],
					'warehouse_id' 		=> $item['warehouse_id'],
					'date' 				=> $data['date'],
					'status' 			=> 'received',
